...
class Robot : public nodelet::Nodelet {
 ...
 private:
  ...
  @return void
  **/
  bool collisionExists(const geometry_msgs::Pose2D& newPose,
  ...
  @param newPose [const geometry_msgs::Pose2D] The pose for the robot to be
  moved to
 ...
 private:
  ...
  std::vector<std::pair<int, int> > getPointsBetween(int x1, int y1, int x2,
                                                     int y2);
