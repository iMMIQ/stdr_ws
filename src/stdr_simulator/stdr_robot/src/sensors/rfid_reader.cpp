...
RfidReader::RfidReader(const nav_msgs::OccupancyGrid& map,
  ...
  rfids_subscriber_ =
      n.subscribe("stdr_server/rfid_list", 1, &RfidReader::receiveRfids, this);
}
...
void RfidReader::updateSensorCallback() {
  ...
  for (unsigned int i = 0; i < rfid_tags_.rfid_tags.size(); i++) {
    ...
    float dist = sqrt(pow(sensor_x - rfid_tags_.rfid_tags[i].pose.x, 2) +
    ...
    measuredTagsMsg.rfid_tags_dbs.push_back(
        1.0);  //!< Needs to change into a realistic measurement
  }
