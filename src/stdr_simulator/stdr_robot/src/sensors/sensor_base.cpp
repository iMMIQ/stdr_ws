...
@param sensorPose [const geometry_msgs::Pose2D&] The sensor's pose relative to
robot
@param sensorFrameId [const std::string&] The sensor's frame id
@param updateFrequency [float] The sensor's update frequnecy
@return void
**/
Sensor::Sensor(const nav_msgs::OccupancyGrid& map, const std::string& name,
               ros::NodeHandle& n, const geometry_msgs::Pose2D& sensorPose,
               const std::string& sensorFrameId, float updateFrequency)
    : _map(map),
      _namespace(name),
      ...
      _gotTransform(false) {
  _timer = n.createTimer(ros::Duration(1 / updateFrequency),
                         &Sensor::checkAndUpdateSensor, this);
  _tfTimer = n.createTimer(ros::Duration(1 / (2 * updateFrequency)),
                           &Sensor::updateTransform, this);
}
...
void Sensor::updateTransform(const ros::TimerEvent&) {
  try {
    ...
    _tfListener.lookupTransform("map_static", _namespace + "_" + _sensorFrameId,
