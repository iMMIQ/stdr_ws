...
**/
ThermalSensor::ThermalSensor(const nav_msgs::OccupancyGrid& map,
  ...
  thermal_sources_subscriber_ =
      n.subscribe("stdr_server/thermal_sources_list", 1,
                  &ThermalSensor::receiveThermalSources, this);
}
...
void ThermalSensor::updateSensorCallback() {
  ...
  for (unsigned int i = 0; i < thermal_sources_.thermal_sources.size(); i++) {
    ...
    float dist =
        sqrt(pow(sensor_x - thermal_sources_.thermal_sources[i].pose.x, 2) +
