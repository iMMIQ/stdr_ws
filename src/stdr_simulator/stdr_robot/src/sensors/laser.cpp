...
Laser::Laser(const nav_msgs::OccupancyGrid& map,
             const stdr_msgs::LaserSensorMsg& msg, const std::string& name,
             ros::NodeHandle& n)
    : Sensor(map, name, n, msg.pose, msg.frame_id, msg.frequency) {
  _description = msg;

  _publisher =
      n.advertise<sensor_msgs::LaserScan>(_namespace + "/" + msg.frame_id, 1);
}
...
void Laser::updateSensorCallback() {
  ...
  for (int laserScanIter = 0; laserScanIter < _description.numRays;
    ...
    angle = tf::getYaw(_sensorTransform.getRotation()) + _description.minAngle +
            laserScanIter * (_description.maxAngle - _description.minAngle) /
                divisions;
