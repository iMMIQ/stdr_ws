...
@return void
**/
SoundSensor::SoundSensor(const nav_msgs::OccupancyGrid& map,
  ...
  sound_sources_subscriber_ =
      n.subscribe("stdr_server/sound_sources_list", 1,
                  &SoundSensor::receiveSoundSources, this);
}
...
void SoundSensor::updateSensorCallback() {
  ...
  for (unsigned int i = 0; i < sound_sources_.sound_sources.size(); i++) {
    ...
    float dist =
        sqrt(pow(sensor_x - sound_sources_.sound_sources[i].pose.x, 2) +
    ...
    float ang = atan2(sound_sources_.sound_sources[i].pose.y - sensor_y,
    ...
    if (dist > 0.5) {
      measuredSourcesMsg.sound_dbs +=
          sound_sources_.sound_sources[i].dbs * pow(0.5, 2) / pow(dist, 2);
    } else {
