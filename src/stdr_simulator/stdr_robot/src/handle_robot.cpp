...
@param msg [const stdr_msgs::RobotMsg] The robot message from which the robot is
created
...
**/
bool HandleRobot::moveRobot(const std::string& name,
                            const geometry_msgs::Pose2D newPose) {
  while (!ros::service::waitForService(name + "/replace", ros::Duration(.1)) &&
         ros::ok()) {
