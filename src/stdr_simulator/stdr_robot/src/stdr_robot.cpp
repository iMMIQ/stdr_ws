...
void Robot::onInit() {
  ...
  goal.name = getName();
  _registerClientPtr->sendGoal(
      goal, boost::bind(&Robot::initializeRobot, this, _1, _2));

  _mapSubscriber = n.subscribe("map", 1, &Robot::mapCallback, this);
  _moveRobotService = n.advertiseService(getName() + "/replace",
                                         &Robot::moveRobotCallback, this);
...
@param result [const stdr_msgs::RegisterRobotResultConstPtr] Action result of
...
void Robot::initializeRobot(
  ...
  for (unsigned int laserIter = 0;
    ...
    _sensors.push_back(SensorPtr(new Laser(
        _map, result->description.laserSensors[laserIter], getName(), n)));
  }
  for (unsigned int sonarIter = 0;
       sonarIter < result->description.sonarSensors.size(); sonarIter++) {
    _sensors.push_back(SensorPtr(new Sonar(
        _map, result->description.sonarSensors[sonarIter], getName(), n)));
  }
  for (unsigned int rfidReaderIter = 0;
       rfidReaderIter < result->description.rfidSensors.size();
       rfidReaderIter++) {
    _sensors.push_back(SensorPtr(new RfidReader(
        _map, result->description.rfidSensors[rfidReaderIter], getName(), n)));
  }
  for (unsigned int co2SensorIter = 0;
       co2SensorIter < result->description.co2Sensors.size(); co2SensorIter++) {
    _sensors.push_back(SensorPtr(new CO2Sensor(
        _map, result->description.co2Sensors[co2SensorIter], getName(), n)));
  }
  for (unsigned int thermalSensorIter = 0;
       thermalSensorIter < result->description.thermalSensors.size();
       thermalSensorIter++) {
    _sensors.push_back(SensorPtr(new ThermalSensor(
        _map, result->description.thermalSensors[thermalSensorIter], getName(),
        n)));
  }
  for (unsigned int soundSensorIter = 0;
    ...
    _sensors.push_back(SensorPtr(
        new SoundSensor(_map, result->description.soundSensors[soundSensorIter],
                        getName(), n)));
  }
  ...
  if (result->description.footprint.points.size() == 0) {
    ...
    for (unsigned int i = 0; i < result->description.footprint.points.size();
         i++) {
  ...
  if (motion_model == "ideal") {
    _motionControllerPtr.reset(new IdealMotionController(
        _currentPose, _tfBroadcaster, n, getName(), p));
  } else if (motion_model == "omni") {
    _motionControllerPtr.reset(new OmniMotionController(
        _currentPose, _tfBroadcaster, n, getName(), p));
  } else {
...
@param newPose [const geometry_msgs::Pose2D] The pose for the robot to be moved
to
...
**/
std::vector<std::pair<int, int> > Robot::getPointsBetween(int x1, int y1,
                                                          int x2, int y2) {
...
bool Robot::collisionExists(const geometry_msgs::Pose2D& newPose,
  ...
  if (_map.info.width == 0 || _map.info.height == 0) return false;
  ...
  while (pow(xMap - x, 2) + pow(yMap - y, 2) > 1) {
    ...
    for (unsigned int i = 0; i < _footprint.size(); i++) {
      ...
      for (unsigned int j = 0; j < pts.size(); j++) {
        static int OF = 1;
        if (_map.data[(pts[j].second - OF) * _map.info.width + pts[j].first -
                      OF] > 70 ||
            _map.data[(pts[j].second - OF) * _map.info.width + pts[j].first] >
                70 ||
            _map.data[(pts[j].second - OF) * _map.info.width + pts[j].first +
                      OF] > 70 ||
            _map.data[(pts[j].second) * _map.info.width + pts[j].first - OF] >
                70 ||
            _map.data[(pts[j].second) * _map.info.width + pts[j].first + OF] >
                70 ||
            _map.data[(pts[j].second + OF) * _map.info.width + pts[j].first -
...
void Robot::publishTransforms(const ros::TimerEvent&) {
  ...
  odom.pose.pose.orientation =
      tf::createQuaternionMsgFromYaw(_previousPose.theta);
  ...
  for (int i = 0; i < _sensors.size(); i++) {
    ...
    _tfBroadcaster.sendTransform(tf::StampedTransform(
        robotToSensor, ros::Time::now(), getName(), _sensors[i]->getFrameId()));
  }
}
