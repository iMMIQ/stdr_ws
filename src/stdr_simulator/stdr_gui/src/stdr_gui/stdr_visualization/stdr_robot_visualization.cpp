...
CRobotVisualisation::CRobotVisualisation(QString name, float resolution)
  ...
  void_image_ =
      QImage(robotImage->width(), robotImage->height(), QImage::Format_RGB32);
...
@return bool
**/
bool CRobotVisualisation::getActive(void) { return active_; }
...
void CRobotVisualisation::setCurrentPose(geometry_msgs::Pose2D pose) {
  robotPoseX->setText(QString("\tx = ") + QString().setNum(pose.x) +
                      QString(" m"));
  robotPoseY->setText(QString("\ty = ") + QString().setNum(pose.y) +
                      QString(" m"));
  robotPoseTheta->setText(QString("\ttheta = ") + QString().setNum(pose.theta) +
                          QString(" rad"));
}
...
void CRobotVisualisation::setCurrentSpeed(std::vector<float> msg) {
  robotSpeedLinear->setText(QString("\tu_x = ") + QString().setNum(msg[0]) +
                            QString(" m/s"));
  robotSpeedLinearY->setText(QString("\tu_y = ") + QString().setNum(msg[1]) +
                             QString(" m/s"));
  robotSpeedAngular->setText(QString("\tw = ") + QString().setNum(msg[2]) +
                             QString(" rad/s"));
}
}  // namespace stdr_gui
