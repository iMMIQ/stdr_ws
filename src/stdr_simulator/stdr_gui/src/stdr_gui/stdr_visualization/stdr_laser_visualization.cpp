...
CLaserVisualisation::CLaserVisualisation(QString name, float resolution)
  ...
  subscriber_ = n.subscribe(name_.toStdString().c_str(), 1,
                            &CLaserVisualisation::callback, this);

  void_image_ =
      QImage(laserImage->width(), laserImage->height(), QImage::Format_RGB32);
...
@return bool
**/
bool CLaserVisualisation::getActive(void) { return active_; }
...
void CLaserVisualisation::paint(void) {
  ...
  for (unsigned int i = 0; i < scan_.ranges.size(); i++) {
    ...
    mean += real_dist;
    painter.drawLine(
        internal_image_.width() / 2, internal_image_.height() / 2,
  ...
  laserMean->setText(QString().setNum(mean / scan_.ranges.size()) +
                     QString(" m"));
