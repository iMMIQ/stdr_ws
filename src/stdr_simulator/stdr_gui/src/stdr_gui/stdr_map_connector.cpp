...
CMapConnector::CMapConnector(int argc, char **argv)
  ...
  QObject::connect(this, SIGNAL(signalUpdateImage(QImage *)), this,
                   SLOT(serveImage(QImage *)));

  QPixmap p((stdr_gui_tools::getRosPackagePath("stdr_gui") +
             std::string("/resources/images/zoom_in.png"))
                .c_str());
  zoom_in_cursor_ = QCursor(p.scaled(20, 20));

  p = QPixmap((stdr_gui_tools::getRosPackagePath("stdr_gui") +
...
bool CMapConnector::eventFilter(QObject *watched, QEvent *event) {
  ...
  if (watched == loader_.map) {
    if (event->type() == QEvent::MouseButtonPress) {
      ...
      const QWheelEvent *const me = static_cast<const QWheelEvent *>(event);
      QPoint p = me->pos();
      if (me->delta() > 0) {
        Q_EMIT zoomInPressed(p);
      } else {
      ...
      const QKeyEvent *const me = static_cast<const QKeyEvent *>(event);
...
@return void
**/
void CMapConnector::updateImage(QImage *img) { Q_EMIT signalUpdateImage(img); }
...
@return void
**/
void CMapConnector::serveImage(QImage *img) { loader_.updateImage(img); }
...
**/
QPoint CMapConnector::mapToGlobal(QPoint p) { return loader_.mapToGlobal(p); }
...
@return void
**/
void CMapConnector::setMapInitialized(bool mi) { map_initialized_ = mi; }
}  // namespace stdr_gui
