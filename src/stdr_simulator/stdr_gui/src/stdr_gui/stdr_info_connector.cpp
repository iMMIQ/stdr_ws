...
CInfoConnector::CInfoConnector(int argc, char **argv)
    : QObject(), loader(argc, argv), argc_(argc), argv_(argv) {
  QObject::connect(loader.stdrInformationTree,
                   SIGNAL(itemClicked(QTreeWidgetItem *, int)), this,
                   SLOT(treeItemClicked(QTreeWidgetItem *, int)));

  QObject::connect(this, SIGNAL(adaptSignal()), this, SLOT(adaptSlot()));
...
/**
@brief Adapts the columns width according to what is visible when an item is
clicked
...
/**
@brief Adapts the columns width according to what is visible when an item
...
/**
@brief Adapts the columns width according to what is visible. Called when
adaptSignal is emmited
@return void
**/
...
void CInfoConnector::treeItemClicked(QTreeWidgetItem *item, int column) {
  ...
  if (item == &loader.robotsInfo || item == &loader.generalInfo) {
    ...
    Q_EMIT laserVisualizerClicked(item->parent()->parent()->text(0),
                                  item->text(0));
  } else if (item->parent()->text(0) == QString("Lasers") && column == 2) {
    Q_EMIT laserVisibilityClicked(item->parent()->parent()->text(0),
                                  item->text(0));
  } else if (item->parent()->text(0) == QString("Sonars") && column == 3) {
    Q_EMIT sonarVisualizerClicked(item->parent()->parent()->text(0),
                                  item->text(0));
  } else if (item->parent()->text(0) == QString("Sonars") && column == 2) {
    Q_EMIT sonarVisibilityClicked(item->parent()->parent()->text(0),
                                  item->text(0));
  } else if (item->parent()->text(0) == QString("RFID readers") &&
             column == 2) {
    Q_EMIT rfidReaderVisibilityClicked(item->parent()->parent()->text(0),
                                       item->text(0));
  } else if (item->parent()->text(0) == QString("CO2 sensors") && column == 2) {
    Q_EMIT co2SensorVisibilityClicked(item->parent()->parent()->text(0),
                                      item->text(0));
  } else if (item->parent()->text(0) == QString("Thermal sensors") &&
             column == 2) {
    Q_EMIT thermalSensorVisibilityClicked(item->parent()->parent()->text(0),
                                          item->text(0));
  } else if (item->parent()->text(0) == QString("Sound sensors") &&
             column == 2) {
    Q_EMIT soundSensorVisibilityClicked(item->parent()->parent()->text(0),
                                        item->text(0));
    ...
    Q_EMIT robotVisibilityClicked(item->text(0));
  }
}
...
@return void
**/
void CInfoConnector::setLaserVisibility(QString robotName, QString laserName,
                                        char vs) {
...
@return void
**/
void CInfoConnector::setSonarVisibility(QString robotName, QString sonarName,
                                        char vs) {
  for (int i = 0; i < loader.robotsInfo.childCount(); i++) {
    QString text = loader.robotsInfo.child(i)->text(0);
    if (text == robotName) {
...
@return void
**/
void CInfoConnector::setRfidReaderVisibility(QString robotName,
...
@return void
**/
void CInfoConnector::setCO2SensorVisibility(QString robotName,
                                            QString sensorName, char vs) {
  for (int i = 0; i < loader.robotsInfo.childCount(); i++) {
    QString text = loader.robotsInfo.child(i)->text(0);
    if (text == robotName) {
      QTreeWidgetItem *it = loader.robotsInfo.child(i);
...
@return void
**/
void CInfoConnector::setThermalSensorVisibility(QString robotName,
                                                QString sensorName, char vs) {
  for (int i = 0; i < loader.robotsInfo.childCount(); i++) {
    QString text = loader.robotsInfo.child(i)->text(0);
    if (text == robotName) {
...
@return void
**/
void CInfoConnector::setSoundSensorVisibility(QString robotName,
                                              QString sensorName, char vs) {
  for (int i = 0; i < loader.robotsInfo.childCount(); i++) {
    QString text = loader.robotsInfo.child(i)->text(0);
    if (text == robotName) {
