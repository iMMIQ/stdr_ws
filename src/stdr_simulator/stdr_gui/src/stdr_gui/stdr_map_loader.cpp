...
@return void
**/
CMapLoader::CMapLoader(int argc, char **argv) : argc_(argc), argv_(argv) {
  setupUi(this);
...
@return void
**/
void CMapLoader::resizeEvent(QResizeEvent *e) { updateImage(internal_img_); }
...
void CMapLoader::updateImage(QImage *img) {
  ...
  map->setPixmap(QPixmap().fromImage(
      (*img)
          .copy(map_min_.x(), map_min_.y(), map_max_.x() - map_min_.x(),
                map_max_.y() - map_min_.y())
          .scaled(newDims.first, newDims.second, Qt::IgnoreAspectRatio,
