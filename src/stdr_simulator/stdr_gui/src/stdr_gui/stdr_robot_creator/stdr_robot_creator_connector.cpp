...
CRobotCreatorConnector::CRobotCreatorConnector(int argc, char **argv)
    : QObject(), loader_(argc, argv), argc_(argc), argv_(argv) {
  QObject::connect(loader_.robotTreeWidget,
                   SIGNAL(itemClicked(QTreeWidgetItem *, int)), this,
                   SLOT(treeItemClicked(QTreeWidgetItem *, int)));

  QObject::connect(loader_.laserPropLoader.laserUpdateButton,
                   SIGNAL(clicked(bool)), this, SLOT(updateLaser()));
  QObject::connect(loader_.laserPropLoader.refresh_laser, SIGNAL(clicked(bool)),
                   this, SLOT(updateLaserOpen()));

  QObject::connect(loader_.robotPropLoader.updateButton, SIGNAL(clicked(bool)),
                   this, SLOT(updateRobot()));
  QObject::connect(loader_.robotPropLoader.refresh_robot, SIGNAL(clicked(bool)),
                   this, SLOT(updateRobotOpen()));

  QObject::connect(loader_.sonarPropLoader.pushButton, SIGNAL(clicked(bool)),
                   this, SLOT(updateSonar()));
  QObject::connect(loader_.sonarPropLoader.refresh_sonar, SIGNAL(clicked(bool)),
                   this, SLOT(updateSonarOpen()));

  QObject::connect(loader_.rfidAntennaPropLoader.pushButton,
                   SIGNAL(clicked(bool)), this, SLOT(updateRfidAntenna()));
  QObject::connect(loader_.rfidAntennaPropLoader.refreshRfid,
                   SIGNAL(clicked(bool)), this, SLOT(updateRfidAntennaOpen()));

  QObject::connect(loader_.co2SensorPropLoader.pushButton,
                   SIGNAL(clicked(bool)), this, SLOT(updateCO2Sensor()));
  QObject::connect(loader_.co2SensorPropLoader.refreshButton,
                   SIGNAL(clicked(bool)), this, SLOT(updateCO2SensorOpen()));

  QObject::connect(loader_.thermalSensorPropLoader.pushButton,
                   SIGNAL(clicked(bool)), this, SLOT(updateThermalSensor()));
  QObject::connect(loader_.thermalSensorPropLoader.refreshButton,
                   SIGNAL(clicked(bool)), this,
                   SLOT(updateThermalSensorOpen()));

  QObject::connect(loader_.soundSensorPropLoader.pushButton,
                   SIGNAL(clicked(bool)), this, SLOT(updateSoundSensor()));
  QObject::connect(loader_.soundSensorPropLoader.refreshButton,
                   SIGNAL(clicked(bool)), this, SLOT(updateSoundSensorOpen()));

  QObject::connect(loader_.robotFootLoader.updateButton, SIGNAL(clicked(bool)),
                   this, SLOT(updateFootprintPoint()));
  QObject::connect(loader_.robotFootLoader.refresh_robot, SIGNAL(clicked(bool)),
                   this, SLOT(updateFootprintPointOpen()));

  QObject::connect(loader_.kinematicPropLoader.updateButton,
                   SIGNAL(clicked(bool)), this, SLOT(updateKinematicModel()));

  QObject::connect(loader_.loadRobotButton, SIGNAL(clicked(bool)), this,
                   SLOT(loadRobot()));
...
@return void
**/
void CRobotCreatorConnector::treeItemClicked(QTreeWidgetItem *item,
                                             int column) {
  //!< Laser clicked
...
void CRobotCreatorConnector::updateFootprintPoint(void) {
  ...
  current_footprint_point_->setText(
      0, QString("[") + xstr + QString(",") + ystr + QString("]"));
...
void CRobotCreatorConnector::addLaser(void) {
  ...
  QTreeWidgetItem *angleSpan, *orientation, *maxRange, *minRange, *noiseMean,
      *noiseStd, *poseX, *poseY, *frequency, *rays;
...
void CRobotCreatorConnector::addSonar(void) {
  ...
  QTreeWidgetItem *coneAngle, *orientation, *maxRange, *minRange, *noiseMean,
      *noiseStd, *poseX, *poseY, *frequency;
...
void CRobotCreatorConnector::addRfidAntenna(void) {
  ...
  QTreeWidgetItem *angleSpan, *orientation, *maxRange, *poseX, *poseY,
      *signalCutoff, *frequency;
...
void CRobotCreatorConnector::addThermalSensor(void) {
  QString thermalSensorFrameId =
      QString("thermal_sensor_") +
...
void CRobotCreatorConnector::addSoundSensor(void) {
  QString soundSensorFrameId =
      QString("sound_sensor_") +
...
@return void
**/
void CRobotCreatorConnector::addThermalSensor(
    stdr_msgs::ThermalSensorMsg smsg) {
...
void CRobotCreatorConnector::eraseLaser(QTreeWidgetItem *item) {
  ...
  new_robot_msg_.laserSensors.erase(new_robot_msg_.laserSensors.begin() +
                                    laserFrameId);
  deleteTreeNode(item);
  updateRobotPreview();
}
...
void CRobotCreatorConnector::eraseSonar(QTreeWidgetItem *item) {
  ...
  new_robot_msg_.sonarSensors.erase(new_robot_msg_.sonarSensors.begin() +
                                    sonarFrameId);
  deleteTreeNode(item);
  updateRobotPreview();
}
...
@param item [QTreeWidgetItem*] Tree item that holds the specific rfid antenna
sensor
@return void
**/
void CRobotCreatorConnector::eraseRfid(QTreeWidgetItem *item) {
  ...
  new_robot_msg_.rfidSensors.erase(new_robot_msg_.rfidSensors.begin() +
                                   rfidFrameId);
  deleteTreeNode(item);
  updateRobotPreview();
}
...
void CRobotCreatorConnector::eraseCO2Sensor(QTreeWidgetItem *item) {
  ...
  new_robot_msg_.co2Sensors.erase(new_robot_msg_.co2Sensors.begin() + frameId);
  deleteTreeNode(item);
  updateRobotPreview();
}
/**
...
void CRobotCreatorConnector::eraseThermalSensor(QTreeWidgetItem *item) {
  ...
  new_robot_msg_.thermalSensors.erase(new_robot_msg_.thermalSensors.begin() +
                                      frameId);
  deleteTreeNode(item);
  updateRobotPreview();
}
...
void CRobotCreatorConnector::eraseSoundSensor(QTreeWidgetItem *item) {
  ...
  new_robot_msg_.soundSensors.erase(new_robot_msg_.soundSensors.begin() +
                                    frameId);
  deleteTreeNode(item);
  updateRobotPreview();
}
...
/**
@brief Edits a specific footprint point based on a tree item. Initiates the
footprint editor widget
...
void CRobotCreatorConnector::editFootprintPoint(QTreeWidgetItem *item) {
  ...
  loader_.robotFootLoader.setWindowTitle(QApplication::translate(
      "Footprint point", item->text(0).toStdString().c_str(), 0,
...
/**
@brief Edits a specific laser sensor based on a tree item. Initiates the laser
sensor editor widget
@param item [QTreeWidgetItem*] Tree item that holds the specific laser sensor
@return void
**/
void CRobotCreatorConnector::editLaser(QTreeWidgetItem *item) {
  ...
  loader_.laserPropLoader.laserAngleSpan->setText(
      QString().setNum(new_robot_msg_.laserSensors[laserFrameId].maxAngle -
  ...
  loader_.laserPropLoader.laserNoiseMean->setText(QString().setNum(
      new_robot_msg_.laserSensors[laserFrameId].noise.noiseMean));

  loader_.laserPropLoader.laserNoiseStd->setText(QString().setNum(
  ...
  loader_.laserPropLoader.setWindowTitle(QApplication::translate(
      "LaserProperties", item->text(0).toStdString().c_str(), 0,
      QApplication::UnicodeUTF8));
...
void CRobotCreatorConnector::saveLaser(QTreeWidgetItem *item) {
  ...
  try {
    stdr_parser::Parser::saveMessage(stdr_gui_tools::fixLaserAnglesToRad(lmsg),
                                     file_name_str);
...
void CRobotCreatorConnector::saveSonar(QTreeWidgetItem *item) {
  ...
  try {
    stdr_parser::Parser::saveMessage(stdr_gui_tools::fixSonarAnglesToRad(smsg),
                                     file_name_str);
  } catch (stdr_parser::ParserException ex) {
    QMessageBox msg;
    msg.setWindowTitle(QString("STDR Parser - Error"));
...
void CRobotCreatorConnector::saveRfidAntenna(QTreeWidgetItem *item) {
  ...
  QString file_name = QFileDialog::getSaveFileName(
      &loader_, tr("Save RFID reader sensor"),
      QString().fromStdString(
          stdr_gui_tools::getRosPackagePath("stdr_resources")) +
          QString("/resources/"),
      tr("Resource files (*.yaml *.xml)"));
  ...
  try {
    stdr_parser::Parser::saveMessage(stdr_gui_tools::fixRfidAnglesToRad(smsg),
                                     file_name_str);
  } catch (stdr_parser::ParserException ex) {
    QMessageBox msg;
    msg.setWindowTitle(QString("STDR Parser - Error"));
...
void CRobotCreatorConnector::saveCO2Sensor(QTreeWidgetItem *item) {
  ...
  try {
    stdr_parser::Parser::saveMessage(stdr_gui_tools::fixCO2AnglesToRad(smsg),
                                     file_name_str);
  } catch (stdr_parser::ParserException ex) {
    QMessageBox msg;
    msg.setWindowTitle(QString("STDR Parser - Error"));
...
void CRobotCreatorConnector::saveThermalSensor(QTreeWidgetItem *item) {
  ...
  QString file_name = QFileDialog::getSaveFileName(
      &loader_, tr("Save Thermal sensor"),
      QString().fromStdString(
          stdr_gui_tools::getRosPackagePath("stdr_resources")) +
          QString("/resources/"),
      tr("Resource files (*.yaml *.xml)"));
...
void CRobotCreatorConnector::saveSoundSensor(QTreeWidgetItem *item) {
  ...
  try {
    stdr_parser::Parser::saveMessage(stdr_gui_tools::fixSoundAnglesToRad(smsg),
                                     file_name_str);
  } catch (stdr_parser::ParserException ex) {
    QMessageBox msg;
    msg.setWindowTitle(QString("STDR Parser - Error"));
...
void CRobotCreatorConnector::loadLaser(QTreeWidgetItem *item) {
  ...
  stdr_msgs::LaserSensorMsg lmsg;
  try {
    lmsg = stdr_parser::Parser::createMessage<stdr_msgs::LaserSensorMsg>(
...
void CRobotCreatorConnector::loadSonar(QTreeWidgetItem *item) {
  ...
  std::string old_frame_id = item->text(0).toStdString();
  try {
    smsg = stdr_parser::Parser::createMessage<stdr_msgs::SonarSensorMsg>(
        file_name.toStdString());
  } catch (stdr_parser::ParserException ex) {
    QMessageBox msg;
    msg.setWindowTitle(QString("STDR Parser - Error"));
...
void CRobotCreatorConnector::loadRfidAntenna(QTreeWidgetItem *item) {
  ...
  QString file_name = QFileDialog::getOpenFileName(
      &loader_, tr("Load RFID reader sensor"),
      QString().fromStdString(
          stdr_gui_tools::getRosPackagePath("stdr_resources")) +
          QString("/resources/"),
      tr("Resource Files (*.yaml *.xml)"));
  ...
  std::string old_frame_id = item->text(0).toStdString();
  try {
    smsg = stdr_parser::Parser::createMessage<stdr_msgs::RfidSensorMsg>(
        file_name.toStdString());
  } catch (stdr_parser::ParserException ex) {
    QMessageBox msg;
    msg.setWindowTitle(QString("STDR Parser - Error"));
...
void CRobotCreatorConnector::loadCO2Sensor(QTreeWidgetItem *item) {
  ...
  std::string old_frame_id = item->text(0).toStdString();
  try {
    smsg = stdr_parser::Parser::createMessage<stdr_msgs::CO2SensorMsg>(
        file_name.toStdString());
  } catch (stdr_parser::ParserException ex) {
    QMessageBox msg;
    msg.setWindowTitle(QString("STDR Parser - Error"));
...
void CRobotCreatorConnector::loadThermalSensor(QTreeWidgetItem *item) {
  ...
  QString file_name = QFileDialog::getOpenFileName(
      &loader_, tr("Load thermal sensor"),
      QString().fromStdString(
          stdr_gui_tools::getRosPackagePath("stdr_resources")) +
          QString("/resources/"),
      tr("Resource Files (*.yaml *.xml)"));
  ...
  std::string old_frame_id = item->text(0).toStdString();
  try {
    smsg = stdr_parser::Parser::createMessage<stdr_msgs::ThermalSensorMsg>(
        file_name.toStdString());
  } catch (stdr_parser::ParserException ex) {
    QMessageBox msg;
    msg.setWindowTitle(QString("STDR Parser - Error"));
...
void CRobotCreatorConnector::loadSoundSensor(QTreeWidgetItem *item) {
  ...
  std::string old_frame_id = item->text(0).toStdString();
  try {
    smsg = stdr_parser::Parser::createMessage<stdr_msgs::SoundSensorMsg>(
        file_name.toStdString());
  } catch (stdr_parser::ParserException ex) {
    QMessageBox msg;
    msg.setWindowTitle(QString("STDR Parser - Error"));
...
/**
@brief Edits a specific sonar sensor based on a tree item. Initiates the sonar
sensor editor widget
@param item [QTreeWidgetItem*] Tree item that holds the specific sonar sensor
@return void
**/
void CRobotCreatorConnector::editSonar(QTreeWidgetItem *item) {
  ...
  loader_.sonarPropLoader.sonarNoiseMean->setText(QString().setNum(
      new_robot_msg_.sonarSensors[sonarFrameId].noise.noiseMean));

  loader_.sonarPropLoader.sonarNoiseStd->setText(QString().setNum(
  ...
  loader_.sonarPropLoader.setWindowTitle(QApplication::translate(
      "SonarProperties", item->text(0).toStdString().c_str(), 0,
      QApplication::UnicodeUTF8));
...
/**
@brief Edits a specific rfid antenna sensor based on a tree item. Initiates the
rfid antenna sensor editor widget
@param item [QTreeWidgetItem*] Tree item that holds the specific rfid antenna
sensor
@return void
**/
void CRobotCreatorConnector::editRfid(QTreeWidgetItem *item) {
  ...
  loader_.rfidAntennaPropLoader.setWindowTitle(QApplication::translate(
      "RfidAntennaProperties", item->text(0).toStdString().c_str(), 0,
      QApplication::UnicodeUTF8));
...
void CRobotCreatorConnector::editCO2Sensor(QTreeWidgetItem *item) {
  ...
  loader_.co2SensorPropLoader.setWindowTitle(QApplication::translate(
      "CO2SensorProperties", item->text(0).toStdString().c_str(), 0,
      QApplication::UnicodeUTF8));
...
void CRobotCreatorConnector::editThermalSensor(QTreeWidgetItem *item) {
  ...
  loader_.thermalSensorPropLoader.setWindowTitle(QApplication::translate(
      "ThermalSensorProperties", item->text(0).toStdString().c_str(), 0,
      QApplication::UnicodeUTF8));
...
void CRobotCreatorConnector::editSoundSensor(QTreeWidgetItem *item) {
  ...
  loader_.soundSensorPropLoader.setWindowTitle(QApplication::translate(
      "SoundSensorProperties", item->text(0).toStdString().c_str(), 0,
      QApplication::UnicodeUTF8));
...
void CRobotCreatorConnector::updateRobotTree(void) {
  for (unsigned int i = 0; i < loader_.robotNode.childCount(); i++) {
    if (loader_.robotNode.child(i)->text(0) == QString("Orientation")) {
      loader_.robotNode.child(i)->setText(
          1, QString().setNum(new_robot_msg_.initialPose.theta));
    }
    if (loader_.robotNode.child(i)->text(0) == QString("Radius")) {
      loader_.robotNode.child(i)->setText(
          1, QString().setNum(new_robot_msg_.footprint.radius));
    }
  }
  int count = 0;
...
@return void
**/
void CRobotCreatorConnector::updateLaserTree(QTreeWidgetItem *item,
...
void CRobotCreatorConnector::updateLaser(void) {
  ...
  for (unsigned int i = 0; i < current_laser_->childCount(); i++) {
    ...
    if (current_laser_->child(i)->text(0) == QString("Angle span")) {
      ...
      float angleSpan =
          loader_.laserPropLoader.laserAngleSpan->text().toFloat() / 2.0;
      if (angleSpan <= 0) {
    ...
    //!< Laser orientation
      ...
      float orientation =
          loader_.laserPropLoader.laserOrientation->text().toFloat();
      current_laser_->child(i)->setText(1, QString().setNum(orientation));
    ...
    //!< Laser max range
      ...
      max_range = loader_.laserPropLoader.laserMaxDistance->text().toFloat();
      if (max_range <= 0) {
    ...
    //!< Laser minimum range
      ...
      min_range = loader_.laserPropLoader.laserMinDistance->text().toFloat();
      if (min_range < 0) {
    ...
    //!< Laser mean noise
      ...
      float noiseMean =
          loader_.laserPropLoader.laserNoiseMean->text().toFloat();
      current_laser_->child(i)->setText(1, QString().setNum(noiseMean));
      new_robot_msg_.laserSensors[laserFrameId].noise.noiseMean = noiseMean;
    }

    //!< Laser std noise
    else if (current_laser_->child(i)->text(0) == QString("Noise std")) {
      float noiseStd = loader_.laserPropLoader.laserNoiseStd->text().toFloat();
      if (noiseStd < 0) {
      ...
      new_robot_msg_.laserSensors[laserFrameId].noise.noiseStd = noiseStd;
    }

    //!< Laser pose - x coordinate
    else if (current_laser_->child(i)->text(0) == QString("Pose - x")) {
      float dx = loader_.laserPropLoader.laserTranslationX->text().toFloat();
    ...
    //!< Laser pose - y coordinate
      ...
      float dy = loader_.laserPropLoader.laserTranslationY->text().toFloat();
    ...
    //!< Laser publishing frequency
      ...
      float frequency =
          loader_.laserPropLoader.laserFrequency->text().toFloat();
      if (frequency <= 0) {
...
void CRobotCreatorConnector::updateSonar(void) {
  ...
  for (unsigned int i = 0; i < current_sonar_->childCount(); i++) {
    ...
    if (current_sonar_->child(i)->text(0) == QString("Cone span")) {
      float cone_span = loader_.sonarPropLoader.sonarConeSpan->text().toFloat();
      if (cone_span <= 0) {
    ...
    //!< Sonar orientation
      ...
      current_sonar_->child(i)->setText(1, QString().setNum(orientation));
    ...
    //!< Sonar max range
      ...
      current_sonar_->child(i)->setText(1, QString().setNum(max_range));
    ...
    //!< Sonar min range
      ...
      if (max_range < min_range) {
        showMessage(QString("Sonar minimum range higher than maximum range."));
        return;
      }
    ...
    //!< Sonar noise mean
      ...
      current_sonar_->child(i)->setText(1, QString().setNum(noiseMean));
    ...
    //!< Sonar noise standard deviation
      ...
      float noiseStd = loader_.sonarPropLoader.sonarNoiseStd->text().toFloat();
      if (noiseStd < 0) {
        showMessage(QString("Sonar noise standard deviation invalid :") +
                    QString().setNum(noiseStd));
        return;
      }
      current_sonar_->child(i)->setText(1, QString().setNum(noiseStd));
...
@return void
**/
void CRobotCreatorConnector::updateSonarTree(QTreeWidgetItem *item,
...
@return void
**/
void CRobotCreatorConnector::updateRfidTree(QTreeWidgetItem *item,
...
**/
void CRobotCreatorConnector::updateCO2SensorTree(QTreeWidgetItem *item,
...
void CRobotCreatorConnector::updateThermalSensorTree(
    QTreeWidgetItem *item, stdr_msgs::ThermalSensorMsg l) {
  unsigned int frameId = searchThermalSensor(item->text(0));
  if (frameId == -1) {
    return;
  }
...
void CRobotCreatorConnector::updateSoundSensorTree(
    QTreeWidgetItem *item, stdr_msgs::SoundSensorMsg l) {
  unsigned int frameId = searchSoundSensor(item->text(0));
  if (frameId == -1) {
    return;
  }
...
void CRobotCreatorConnector::updateRfidAntenna(void) {
  ...
  for (unsigned int i = 0; i < current_rfid_->childCount(); i++) {
    ...
    //!< Rfid antenna orientation
      ...
      float orientation =
          loader_.rfidAntennaPropLoader.rfidOrientation->text().toFloat();
      current_rfid_->child(i)->setText(1, QString().setNum(orientation));
    ...
    //!< Rfid antenna max range
      ...
      float maxRange =
          loader_.rfidAntennaPropLoader.rfidMaxDistance->text().toFloat();
      if (maxRange <= 0) {
    ...
    //!< Rfid antenna signal cutoff
      ...
      float signal =
          loader_.rfidAntennaPropLoader.rfidSignalCutoff->text().toFloat();
    ...
    //!< Rfid antenna publishing frequency
      ...
      float frequency =
          loader_.rfidAntennaPropLoader.rfidFrequency->text().toFloat();
      if (frequency <= 0) {
...
void CRobotCreatorConnector::updateCO2Sensor(void) {
  ...
  for (unsigned int i = 0; i < current_co2_sensor_->childCount(); i++) {
    ...
    if (current_co2_sensor_->child(i)->text(0) == QString("Orientation")) {
      float orientation =
          loader_.co2SensorPropLoader.orientation->text().toFloat();
      current_co2_sensor_->child(i)->setText(1, QString().setNum(orientation));
    ...
    //!< Rfid antenna max range
      ...
      float maxRange =
          loader_.co2SensorPropLoader.maxDistance->text().toFloat();
      if (maxRange <= 0) {
    ...
    //!< Publishing frequency
      ...
      float frequency = loader_.co2SensorPropLoader.frequency->text().toFloat();
      if (frequency <= 0) {
...
void CRobotCreatorConnector::updateThermalSensor(void) {
  ...
  for (unsigned int i = 0; i < current_thermal_sensor_->childCount(); i++) {
    ...
    if (current_thermal_sensor_->child(i)->text(0) == QString("Angle span")) {
      ...
      current_thermal_sensor_->child(i)->setText(1,
                                                 QString().setNum(angle_span));
      new_robot_msg_.thermalSensors[frameId].angleSpan = angle_span;
    }

    //!< Orientation
    else if (current_thermal_sensor_->child(i)->text(0) ==
             QString("Orientation")) {
      float orientation =
          loader_.thermalSensorPropLoader.orientation->text().toFloat();
      current_thermal_sensor_->child(i)->setText(1,
                                                 QString().setNum(orientation));
    ...
    //!< Rfid antenna max range
             ...
             QString("Max range")) {
      float maxRange =
          loader_.thermalSensorPropLoader.maxDistance->text().toFloat();
      if (maxRange <= 0) {
    ...
    //!< Publishing frequency
             ...
             QString("Frequency")) {
      float frequency =
          loader_.thermalSensorPropLoader.frequency->text().toFloat();
      if (frequency <= 0) {
...
void CRobotCreatorConnector::updateSoundSensor(void) {
  ...
  for (unsigned int i = 0; i < current_sound_sensor_->childCount(); i++) {
    ...
    //!< Orientation
    else if (current_sound_sensor_->child(i)->text(0) ==
             QString("Orientation")) {
      float orientation =
          loader_.soundSensorPropLoader.orientation->text().toFloat();
      current_sound_sensor_->child(i)->setText(1,
                                               QString().setNum(orientation));
    ...
    //!< Rfid antenna max range
      ...
      float maxRange =
          loader_.soundSensorPropLoader.maxDistance->text().toFloat();
      if (maxRange <= 0) {
    ...
    //!< Publishing frequency
      ...
      float frequency =
          loader_.soundSensorPropLoader.frequency->text().toFloat();
      if (frequency <= 0) {
...
void CRobotCreatorConnector::editRobot(void) {
  loader_.robotPropLoader.robotOrientation->setText(
      QString().setNum(new_robot_msg_.initialPose.theta));
  loader_.robotPropLoader.robotRadius->setText(
      QString().setNum(new_robot_msg_.footprint.radius));
...
void CRobotCreatorConnector::updateRobot(void) {
  for (unsigned int i = 0; i < loader_.robotNode.childCount(); i++) {
    if (loader_.robotNode.child(i)->text(0) == QString("Orientation")) {
      loader_.robotNode.child(i)->setText(
          1, loader_.robotPropLoader.robotOrientation->text());
      new_robot_msg_.initialPose.theta =
          loader_.robotPropLoader.robotOrientation->text().toFloat();
    }
    if (loader_.robotNode.child(i)->text(0) == QString("Radius")) {
      loader_.robotNode.child(i)->setText(
          1, loader_.robotPropLoader.robotRadius->text());
...
void CRobotCreatorConnector::drawRobot(void) {
  ...
  if (new_robot_msg_.footprint.points.size() == 0) {
    painter.drawEllipse(250 - new_robot_msg_.footprint.radius * climax_,
    ...
    painter.drawLine(
        250, 250,
        250 + new_robot_msg_.footprint.radius * climax_ * 1.05 *
    ...
    for (unsigned int i = 0; i < new_robot_msg_.footprint.points.size() + 1;
         i++) {
      float x = new_robot_msg_.footprint
                    .points[i % new_robot_msg_.footprint.points.size()]
                    .x;
      float y = -new_robot_msg_.footprint
                     .points[i % new_robot_msg_.footprint.points.size()]
                     .y;
...
void CRobotCreatorConnector::drawLasers(void) {
  ...
  for (unsigned int i = 0; i < new_robot_msg_.laserSensors.size(); i++) {
    ...
    float newx =
        laserx * cos(robotOrientation) - lasery * sin(robotOrientation);
    float newy =
        laserx * sin(robotOrientation) + lasery * cos(robotOrientation);

    painter.drawPie(250 - new_robot_msg_.laserSensors[i].maxRange * climax_ +
                        newx * climax_,
...
void CRobotCreatorConnector::drawSonars(void) {
  ...
  for (unsigned int i = 0; i < new_robot_msg_.sonarSensors.size(); i++) {
    ...
    float newx =
        sonarx * cos(robotOrientation) - sonary * sin(robotOrientation);
    float newy =
        sonarx * sin(robotOrientation) + sonary * cos(robotOrientation);

    painter.drawPie(250 - new_robot_msg_.sonarSensors[i].maxRange * climax_ +
                        newx * climax_,
...
void CRobotCreatorConnector::drawCO2Sensors(void) {
  ...
  for (unsigned int i = 0; i < new_robot_msg_.co2Sensors.size(); i++) {
    ...
    float newx = x * cos(robotOrientation) - y * sin(robotOrientation);
    float newy = x * sin(robotOrientation) + y * cos(robotOrientation);
    ...
    painter.drawPie(
        ...
        new_robot_msg_.co2Sensors[i].maxRange * climax_ * 2, 0,
        (2 * 180.0) * 16);
  }
  loader_.robotPreviewLabel->setPixmap(
      QPixmap().fromImage(loader_.robotPreviewImage));
...
void CRobotCreatorConnector::drawThermalSensors(void) {
  ...
  for (unsigned int i = 0; i < new_robot_msg_.thermalSensors.size(); i++) {
    ...
    painter.drawPie(250 - new_robot_msg_.thermalSensors[i].maxRange * climax_ +
                        newx * climax_,
                    250 - new_robot_msg_.thermalSensors[i].maxRange * climax_ -
                        newy * climax_,
...
void CRobotCreatorConnector::drawSoundSensors(void) {
  ...
  for (unsigned int i = 0; i < new_robot_msg_.soundSensors.size(); i++) {
    ...
    painter.drawPie(250 - new_robot_msg_.soundSensors[i].maxRange * climax_ +
                        newx * climax_,
                    250 - new_robot_msg_.soundSensors[i].maxRange * climax_ -
                        newy * climax_,
...
void CRobotCreatorConnector::saveRobot(void) {
  QString file_name = QFileDialog::getSaveFileName(
      &loader_, tr("Save File"),
      QString().fromStdString(
          stdr_gui_tools::getRosPackagePath("stdr_resources")) +
          QString("/resources/"),
      tr("Resource files (*.yaml *.xml)"));

  Q_EMIT saveRobotPressed(stdr_gui_tools::fixRobotAnglesToRad(new_robot_msg_),
                          file_name);
}
...
void CRobotCreatorConnector::getRobotFromYaml(void) {
  ...
  try {
    new_robot_msg_ = stdr_parser::Parser::createMessage<stdr_msgs::RobotMsg>(
        file_name.toStdString());
  } catch (stdr_parser::ParserException ex) {
    QMessageBox msg;
    msg.setWindowTitle(QString("STDR Parser - Error"));
