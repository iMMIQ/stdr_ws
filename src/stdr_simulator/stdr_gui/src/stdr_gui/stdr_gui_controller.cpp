...
@return void
**/
void spinThreadFunction(void) { ros::spin(); }
...
CGuiController::CGuiController(int argc, char **argv)
  ...
  icon_move_.addFile(
      QString::fromUtf8((stdr_gui_tools::getRosPackagePath("stdr_gui") +
                         std::string("/resources/images/arrow_move.png"))
                            .c_str()),
      QSize(20, 20), QIcon::Normal, QIcon::Off);

  icon_delete_.addFile(
      QString::fromUtf8((stdr_gui_tools::getRosPackagePath("stdr_gui") +
...
void CGuiController::initializeCommunications(void) {
  map_subscriber_ = n_.subscribe("map", 1, &CGuiController::receiveMap, this);

  //!< Rfid related
  rfids_subscriber_ = n_.subscribe("stdr_server/rfid_list", 1,
                                   &CGuiController::receiveRfids, this);
  ...
  co2_sources_subscriber_ =
      n_.subscribe("stdr_server/co2_sources_list", 1,
                   &CGuiController::receiveCO2Sources, this);
  new_co2_source_client_ =
      n_.serviceClient<stdr_msgs::AddCO2Source>("stdr_server/add_co2_source");
  delete_co2_source_client_ = n_.serviceClient<stdr_msgs::DeleteCO2Source>(
      "stdr_server/delete_co2_source");

  //!< Thermal related
  thermal_sources_subscriber_ =
      n_.subscribe("stdr_server/thermal_sources_list", 1,
                   &CGuiController::receiveThermalSources, this);
  new_thermal_source_client_ = n_.serviceClient<stdr_msgs::AddThermalSource>(
  ...
  sound_sources_subscriber_ =
      n_.subscribe("stdr_server/sound_sources_list", 1,
                   &CGuiController::receiveSoundSources, this);
  new_sound_source_client_ = n_.serviceClient<stdr_msgs::AddSoundSource>(
      "stdr_server/add_sound_source");
  delete_sound_source_client_ = n_.serviceClient<stdr_msgs::DeleteSoundSource>(
      "stdr_server/delete_sound_source");

  QObject::connect(&gui_connector_, SIGNAL(setZoomInCursor(bool)),
                   &map_connector_, SLOT(setCursorZoomIn(bool)));

  QObject::connect(&gui_connector_, SIGNAL(setZoomOutCursor(bool)),
                   &map_connector_, SLOT(setCursorZoomOut(bool)));

  QObject::connect(&gui_connector_, SIGNAL(setAdjustedCursor(bool)),
                   &map_connector_, SLOT(setCursorAdjusted(bool)));

  QObject::connect(&map_connector_, SIGNAL(zoomInPressed(QPoint)), this,
                   SLOT(zoomInPressed(QPoint)));

  QObject::connect(&map_connector_, SIGNAL(zoomOutPressed(QPoint)), this,
                   SLOT(zoomOutPressed(QPoint)));

  QObject::connect(&map_connector_,
                   SIGNAL(itemClicked(QPoint, Qt::MouseButton)), this,
                   SLOT(itemClicked(QPoint, Qt::MouseButton)));

  QObject::connect(&info_connector_,
                   SIGNAL(laserVisualizerClicked(QString, QString)), this,
                   SLOT(laserVisualizerClicked(QString, QString)));

  QObject::connect(&info_connector_,
                   SIGNAL(sonarVisualizerClicked(QString, QString)), this,
                   SLOT(sonarVisualizerClicked(QString, QString)));

  QObject::connect(&info_connector_, SIGNAL(robotVisualizerClicked(QString)),
                   this, SLOT(robotVisualizerClicked(QString)));

  QObject::connect(&(gui_connector_.robotCreatorConn),
                   SIGNAL(saveRobotPressed(stdr_msgs::RobotMsg, QString)), this,
                   SLOT(saveRobotPressed(stdr_msgs::RobotMsg, QString)));

  QObject::connect(&(gui_connector_.robotCreatorConn),
                   SIGNAL(loadRobotPressed(stdr_msgs::RobotMsg)), this,
                   SLOT(loadRobotPressed(stdr_msgs::RobotMsg)));

  QObject::connect(&(gui_connector_),
                   SIGNAL(robotFromFile(stdr_msgs::RobotMsg)), this,
                   SLOT(loadRobotFromFilePressed(stdr_msgs::RobotMsg)));

  QObject::connect(&(gui_connector_), SIGNAL(loadRfidPressed()), this,
                   SLOT(loadRfidPressed()));

  QObject::connect(&(gui_connector_), SIGNAL(loadThermalPressed()), this,
                   SLOT(loadThermalPressed()));

  QObject::connect(&(gui_connector_), SIGNAL(loadCo2Pressed()), this,
                   SLOT(loadCo2Pressed()));

  QObject::connect(&(gui_connector_), SIGNAL(loadSoundPressed()), this,
                   SLOT(loadSoundPressed()));

  QObject::connect(this, SIGNAL(waitForRobotPose()), &map_connector_,
                   SLOT(waitForPlace()));

  QObject::connect(&map_connector_, SIGNAL(robotPlaceSet(QPoint)), this,
                   SLOT(robotPlaceSet(QPoint)));

  QObject::connect(this, SIGNAL(replaceRobot(std::string)), &map_connector_,
                   SLOT(waitForReplace(std::string)));

  QObject::connect(&map_connector_,
                   SIGNAL(robotReplaceSet(QPoint, std::string)), this,
                   SLOT(robotReplaceSet(QPoint, std::string)));

  timer_ = new QTimer(this);
  connect(timer_, SIGNAL(timeout()), this, SLOT(updateMapInternal()));

  QObject::connect(this, SIGNAL(waitForRfidPose()), &map_connector_,
                   SLOT(waitForRfidPlace()));

  QObject::connect(&map_connector_, SIGNAL(rfidPlaceSet(QPoint)), this,
                   SLOT(rfidPlaceSet(QPoint)));

  QObject::connect(this, SIGNAL(waitForThermalPose()), &map_connector_,
                   SLOT(waitForThermalPlace()));

  QObject::connect(&map_connector_, SIGNAL(thermalPlaceSet(QPoint)), this,
                   SLOT(thermalPlaceSet(QPoint)));

  QObject::connect(this, SIGNAL(waitForCo2Pose()), &map_connector_,
                   SLOT(waitForCo2Place()));

  QObject::connect(this, SIGNAL(waitForSoundPose()), &map_connector_,
                   SLOT(waitForSoundPlace()));

  QObject::connect(&map_connector_, SIGNAL(soundPlaceSet(QPoint)), this,
                   SLOT(soundPlaceSet(QPoint)));

  QObject::connect(&map_connector_, SIGNAL(co2PlaceSet(QPoint)), this,
                   SLOT(co2PlaceSet(QPoint)));

  QObject::connect(&info_connector_,
                   SIGNAL(laserVisibilityClicked(QString, QString)), this,
                   SLOT(laserVisibilityClicked(QString, QString)));

  QObject::connect(&info_connector_,
                   SIGNAL(sonarVisibilityClicked(QString, QString)), this,
                   SLOT(sonarVisibilityClicked(QString, QString)));

  QObject::connect(&info_connector_,
                   SIGNAL(rfidReaderVisibilityClicked(QString, QString)), this,
                   SLOT(rfidReaderVisibilityClicked(QString, QString)));

  QObject::connect(&info_connector_,
                   SIGNAL(co2SensorVisibilityClicked(QString, QString)), this,
                   SLOT(co2SensorVisibilityClicked(QString, QString)));

  QObject::connect(&info_connector_,
                   SIGNAL(thermalSensorVisibilityClicked(QString, QString)),
                   this,
                   SLOT(thermalSensorVisibilityClicked(QString, QString)));

  QObject::connect(&info_connector_,
                   SIGNAL(soundSensorVisibilityClicked(QString, QString)), this,
                   SLOT(soundSensorVisibilityClicked(QString, QString)));

  QObject::connect(&info_connector_, SIGNAL(robotVisibilityClicked(QString)),
                   this, SLOT(robotVisibilityClicked(QString)));

  QObject::connect(this, SIGNAL(setRobotVisibility(QString, char)),
                   &info_connector_, SLOT(setRobotVisibility(QString, char)));

  QObject::connect(this, SIGNAL(setLaserVisibility(QString, QString, char)),
                   &info_connector_,
                   SLOT(setLaserVisibility(QString, QString, char)));

  QObject::connect(this, SIGNAL(setSonarVisibility(QString, QString, char)),
                   &info_connector_,
                   SLOT(setSonarVisibility(QString, QString, char)));
  ...
  QObject::connect(this, SIGNAL(setCO2SensorVisibility(QString, QString, char)),
                   &info_connector_,
                   SLOT(setCO2SensorVisibility(QString, QString, char)));

  QObject::connect(this,
                   SIGNAL(setThermalSensorVisibility(QString, QString, char)),
                   &info_connector_,
                   SLOT(setThermalSensorVisibility(QString, QString, char)));
...
void CGuiController::setupWidgets(void) {
  { gui_connector_.addToGrid(info_connector_.getLoader(), 0, 0); }
  {
    initial_map_ = running_map_ =
        QImage((stdr_gui_tools::getRosPackagePath("stdr_gui") +
...
void CGuiController::receiveRfids(const stdr_msgs::RfidTagVector &msg) {
  ...
  for (unsigned int i = 0; i < msg.rfid_tags.size(); i++) {
    ...
    CGuiRfidTag temp_tag(p, msg.rfid_tags[i].tag_id, map_msg_.info.resolution);
...
void CGuiController::receiveMap(const nav_msgs::OccupancyGrid &msg) {
  ...
  robot_subscriber_ = n_.subscribe("stdr_server/active_robots", 1,
                                   &CGuiController::receiveRobots, this);
}

/**
@brief Saves the robot in a file. Connects to the
CGuiConnector::CRobotCreatorConnector::saveRobotPressed signal
...
/**
@brief Loads a robot from robot creator into map. Connects to the
CGuiConnector::CRobotCreatorConnector::loadRobotPressed signal
...
/**
@brief Informs CGuiController that a robot is loaded from a yaml file. Connects
to the CGuiConnector::robotFromFile signal
@param newRobotMsg [stdr_msgs::RobotMsg] The robot to be put in the map
@return void
**/
...
/**
@brief Informs CGuiController that an RFID is going to be placed in the
environment. Connects to the CGuiConnector::loadRfidPressed signal
@return void
**/
...
/**
@brief Informs CGuiController that a thermal source is going to be placed in the
environment. Connects to the CGuiConnector::loadThermalPressed signal
@return void
**/
...
/**
@brief Informs CGuiController that a CO2 source is going to be placed in the
environment. Connects to the CGuiConnector::loadCo2Pressed signal
@return void
**/
...
/**
@brief Informs CGuiController that a sound source is going to be placed in the
environment. Connects to the CGuiConnector::loadSoundPressed signal
@return void
**/
...
/**
@brief Performs zoom in when the button is pressed. Connects to the
CMapConnector::zoomInPressed signal
...
/**
@brief Performs zoom out when the button is pressed. Connects to the
CMapConnector::zoomOutPressed signal
@param p [QPoint] The event point in the OGM
@return void
**/
...
/**
@brief Dumps all visualizers that connect to robots not existent in the input
argument message
...
void CGuiController::cleanupVisualizers(
  ...
  for (unsigned int r = 0; r < msg.robots.size(); r++) {
    ...
    for (unsigned int l = 0; l < msg.robots[r].robot.laserSensors.size(); l++) {
      QString fullName =
          baseName + QString("/") +
          QString(msg.robots[r].robot.laserSensors[l].frame_id.c_str());
      newSensors.insert(fullName);
    }
    for (unsigned int s = 0; s < msg.robots[r].robot.sonarSensors.size(); s++) {
      QString fullName =
          baseName + QString("/") +
          QString(msg.robots[r].robot.sonarSensors[s].frame_id.c_str());
      newSensors.insert(fullName);
    }
  }
...
/**
@brief Receives the robots from stdr_server. Connects to
"stdr_server/active_robots" ROS topic
@param msg [const stdr_msgs::RobotIndexedVectorMsg&] The robots message
@return void
**/
...
/**
@brief Gets the point at which the new robot is placed. Connects to the
CMapConnector::robotPlaceSet signal
@param p [QPoint] The event point in the OGM
@return void
**/
void CGuiController::robotPlaceSet(QPoint p) {
  ...
  gui_connector_.robotCreatorConn.setInitialPose(
      pnew.x() * map_msg_.info.resolution, pnew.y() * map_msg_.info.resolution);

  stdr_msgs::RobotIndexedMsg newRobot;
  try {
    newRobot = robot_handler_.spawnNewRobot(stdr_gui_tools::fixRobotAnglesToRad(
...
void CGuiController::rfidPlaceSet(QPoint p) {
  ...
  QString rfid_id = QInputDialog::getText(
      ...
      tr("RFID tag id:"), QLineEdit::Normal, "", &ok);
  ...
  QString rfid_message = QInputDialog::getText(
      &(info_connector_.loader), tr("QInputDialog::getText()"),
      tr("RFID tag message (optional):"), QLineEdit::Normal, "", &ok);
  ...
  if (new_rfid_tag_client_.call(srv)) {
    gui_connector_.raiseMessage("STDR robot - Error",
                                QString(srv.response.message.c_str()));
  }
}
...
void CGuiController::co2PlaceSet(QPoint p) {
  ...
  QString id = QInputDialog::getText(
      &(info_connector_.loader), tr("QInputDialog::getText()"),
      tr("CO2 Source id:"), QLineEdit::Normal, "", &ok);
  if (ok && !id.isEmpty()) {
  ...
  QString ppms = QInputDialog::getText(
      &(info_connector_.loader), tr("QInputDialog::getText()"),
      tr("PPMs (Part Per Million):"), QLineEdit::Normal, "", &ok);
  if (ok && !ppms.isEmpty()) {
...
/**
@brief Gets the point at which the new thermal source is placed. Connects to
the\ CMapConnector::thermalPlaceSet signal
@param p [QPoint] The event point in the OGM
@return void
**/
void CGuiController::thermalPlaceSet(QPoint p) {
  ...
  QPoint pnew = map_connector_.getGlobalPoint(p);
  QString name =
      QString("thermal_source_") + QString().setNum(thermal_sources_.size());
  ...
  QString id = QInputDialog::getText(
      &(info_connector_.loader), tr("QInputDialog::getText()"),
      tr("Thermal Source id:"), QLineEdit::Normal, "", &ok);
  if (ok && !id.isEmpty()) {
    new_source.id = id.toStdString();
  }
  //!< Getting source ppms
  QString degrees = QInputDialog::getText(
      &(info_connector_.loader), tr("QInputDialog::getText()"), tr("Degrees:"),
      QLineEdit::Normal, "", &ok);
...
void CGuiController::soundPlaceSet(QPoint p) {
  ...
  QPoint pnew = map_connector_.getGlobalPoint(p);
  QString name =
      QString("sound_source_") + QString().setNum(sound_sources_.size());
  ...
  QString id = QInputDialog::getText(
      &(info_connector_.loader), tr("QInputDialog::getText()"),
      tr("Sound Source id:"), QLineEdit::Normal, "", &ok);
  if (ok && !id.isEmpty()) {
    new_source.id = id.toStdString();
  }
  //!< Getting source ppms
  QString dbs = QInputDialog::getText(
      &(info_connector_.loader), tr("QInputDialog::getText()"),
      tr("DBs (Decibels):"), QLineEdit::Normal, "", &ok);
  if (ok && !dbs.isEmpty()) {
...
/**
@brief Updates the map to be shown in GUI. Connects to the timeout signal of
timer_
@return void
**/
void CGuiController::updateMapInternal(void) {
  ...
  for (unsigned int i = 0; i < registered_robots_.size(); i++) {
    registered_robots_[i].draw(&running_map_, map_msg_.info.resolution,
                               &listener_);
  }
  running_map_ = running_map_.mirrored(false, true);
  for (unsigned int i = 0; i < registered_robots_.size(); i++) {
    if (registered_robots_[i].getShowLabel())
      registered_robots_[i].drawLabel(&running_map_, map_msg_.info.resolution);
  }

  for (unsigned int i = 0; i < registered_robots_.size(); i++) {
    if (registered_robots_[i].getFrameId() == robot_following_) {
      map_connector_.updateCenter(registered_robots_[i].getCurrentPose());
    }
  }
  ...
  for (Co2SourcesIterator it = co2_sources_.begin(); it != co2_sources_.end();
       it++) {
    it->second.draw(&running_map_);
  }
    ...
    if (!it->second->getActive()) {
      ...
      for (unsigned int r = 0; r < registered_robots_.size(); r++) {
        if (registered_robots_[r].getFrameId() == robotName.toStdString()) {
          ...
          it->second->setCurrentSpeed(registered_robots_[r].getSpeeds());
          break;
        }
      }
    }
  ...
  for (unsigned int i = 0; i < all_robots_.robots.size(); i++) {
    if (all_robots_.robots[i].name == robotName.toStdString()) {
      for (unsigned int j = 0;
           j < all_robots_.robots[i].robot.laserSensors.size(); j++) {
...
stdr_msgs::SonarSensorMsg CGuiController::getSonarDescription(
  ...
  for (unsigned int i = 0; i < all_robots_.robots.size(); i++) {
    if (all_robots_.robots[i].name == robotName.toStdString()) {
      for (unsigned int j = 0;
           j < all_robots_.robots[i].robot.sonarSensors.size(); j++) {
...
/**
@brief Informs CGuiController that a laser visualizer has been clicked. Connects
to the CInfoConnector::laserVisualizerClicked signal
@param robotName [QString] Frame id of the robot
@param laserName [QString] Frame id of the laser
@return void
**/
void CGuiController::laserVisualizerClicked(QString robotName,
                                            QString laserName) {
...
/**
@brief Informs CGuiController that a sonar visualizer has been clicked. Connects
to the CInfoConnector::sonarVisualizerClicked signal
@param robotName [QString] Frame id of the robot
@param sonarName [QString] Frame id of the sonar
@return void
**/
void CGuiController::sonarVisualizerClicked(QString robotName,
                                            QString sonarName) {
...
/**
@brief Informs CGuiController that a robot visualizer has been clicked. Connects
to the CInfoConnector::robotVisualizerClicked signal
@param robotName [QString] Frame id of the robot
@return void
**/
...
/**
@brief Informs CGuiController that click has performed in the map. Connects to
the CMapConnector::itemClicked signal
...
void CGuiController::itemClicked(QPoint p, Qt::MouseButton b) {
    ...
    if (registered_robots_[i].checkEventProximity(pointClicked)) {
      if (b == Qt::RightButton) {
        ...
        if (selectedItem == showCircle) {
          ...
          robot_handler_.deleteRobot(registered_robots_[i].getFrameId());
  ...
  for (RfidTagIterator i = rfid_tags_.begin(); i != rfid_tags_.end(); i++) {
    if (i->second.checkProximity(pointClicked)) {
      if (b == Qt::RightButton) {
        ...
        QAction *name =
            myMenu.addAction(QString("RFID tag : ") + QString(i->first));
        name->setCheckable(false);
        name->setEnabled(false);

        QAction *message = myMenu.addAction(QString("Message : ") +
                                            QString(i->second.getMessage()));
  ...
  for (Co2SourcesIterator i = co2_sources_.begin(); i != co2_sources_.end();
       i++) {
    if (i->second.checkProximity(pointClicked)) {
      if (b == Qt::RightButton) {
        QMenu myMenu;
        ...
        QAction *name =
            myMenu.addAction(QString("CO2 Source : ") + QString(i->first));
        name->setCheckable(false);
        name->setEnabled(false);
        ...
        QAction *deleteSource =
            myMenu.addAction(icon_delete_, "Delete CO2 Source");
  ...
  for (ThermalSourcesIterator i = thermal_sources_.begin();
    ...
    if (i->second.checkProximity(pointClicked)) {
      if (b == Qt::RightButton) {
        ...
        QAction *name =
            myMenu.addAction(QString("Thermal Source : ") + QString(i->first));
        name->setCheckable(false);
        name->setEnabled(false);
        ...
        QAction *deleteSource =
            myMenu.addAction(icon_delete_, "Delete Thermal Source");
  ...
  for (SoundSourcesIterator i = sound_sources_.begin();
    ...
    if (i->second.checkProximity(pointClicked)) {
      if (b == Qt::RightButton) {
        ...
        QAction *name =
            myMenu.addAction(QString("Sound Source : ") + QString(i->first));
        name->setCheckable(false);
        name->setEnabled(false);
        ...
        QAction *deleteSource =
            myMenu.addAction(icon_delete_, "Delete Sound Source");
...
/**
@brief Informs CGuiController about the new pose of a robot. Connects to the
CMapConnector::robotReplaceSet signal
@param p [QPoint] The event point in map
...
/**
@brief Informs CGuiController that a laser visibility status has been clicked.
Connects to the CInfoConnector::laserVisibilityClicked signal
@param robotName [QString] Frame id of the robot
@param laserName [QString] Frame id of the laser
@return void
**/
void CGuiController::laserVisibilityClicked(QString robotName,
                                            QString laserName) {
  for (unsigned int i = 0; i < registered_robots_.size(); i++) {
...
/**
@brief Informs CGuiController that a sonar visibility status has been clicked.
Connects to the CInfoConnector::sonarVisibilityClicked signal
@param robotName [QString] Frame id of the robot
@param sonarName [QString] Frame id of the sonar
@return void
**/
void CGuiController::sonarVisibilityClicked(QString robotName,
                                            QString sonarName) {
  for (unsigned int i = 0; i < registered_robots_.size(); i++) {
    if (registered_robots_[i].getFrameId() == robotName.toStdString()) {
...
signal
**/
void CGuiController::rfidReaderVisibilityClicked(QString robotName,
...
signal
**/
void CGuiController::co2SensorVisibilityClicked(QString robotName,
...
signal
**/
void CGuiController::thermalSensorVisibilityClicked(QString robotName,
                                                    QString thermalSensorName) {
  for (unsigned int i = 0; i < registered_robots_.size(); i++) {
    if (registered_robots_[i].getFrameId() == robotName.toStdString()) {
      char vs = registered_robots_[i].getThermalSensorVisualizationStatus(
          thermalSensorName.toStdString());
      Q_EMIT setThermalSensorVisibility(robotName, thermalSensorName,
                                        (vs + 1) % 3);
...
signal
**/
void CGuiController::soundSensorVisibilityClicked(QString robotName,
                                                  QString soundSensorName) {
  for (unsigned int i = 0; i < registered_robots_.size(); i++) {
    if (registered_robots_[i].getFrameId() == robotName.toStdString()) {
      char vs = registered_robots_[i].getSoundSensorVisualizationStatus(
          soundSensorName.toStdString());
      Q_EMIT setSoundSensorVisibility(robotName, soundSensorName, (vs + 1) % 3);
...
/**
@brief Informs CGuiController that a robot visibility status has been clicked.
Connects to the CInfoConnector::robotVisibilityClicked signal
@param robotName [QString] Frame id of the robot
@return void
**/
