...
@return void
**/
char CGuiLaser::getVisualizationStatus(void) { return visualization_status_; }
...
@return void
**/
void CGuiLaser::paint(QImage *m, float ocgd, tf::TransformListener *listener) {
  lock_ = true;
  QPainter painter(m);
  ...
  try {
    listener->waitForTransform("map_static", tf_frame_.c_str(), ros::Time(0),
                               ros::Duration(0.2));
    listener->lookupTransform("map_static", tf_frame_.c_str(), ros::Time(0),
                              transform);
  ...
  for (unsigned int i = 0; i < scan_.ranges.size(); i++) {
    ...
    if (real_dist > msg_.maxRange) {
      ...
      painter.setPen(QColor(100, 100, 100, 75 * (2 - visualization_status_)));
    } else {
      painter.setPen(QColor(255, 0, 0, 75 * (2 - visualization_status_)));
    }
    painter.drawLine(pose_x / ocgd,

                     pose_y / ocgd,

                     pose_x / ocgd + real_dist *
                                         cos(pose_theta + scan_.angle_min +
                                             i * scan_.angle_increment) /
                                         ocgd,

                     pose_y / ocgd + real_dist *
                                         sin(pose_theta + scan_.angle_min +
                                             i * scan_.angle_increment) /
                                         ocgd);
  }
  lock_ = false;
...
@param m [QImage*] The image to be drawn
@param ocgd [float] The map's resolution
@param maxRange [float] The maximum range of all the robot sensors. Used for the
visualizer proportions
@return void
**/
void CGuiLaser::visualizerPaint(QImage *m, float ocgd, float maxRange) {
  lock_ = true;
  QPainter painter(m);
  float size = m->width();
  ...
  for (unsigned int i = 0; i < scan_.ranges.size(); i++) {
    ...
    painter.drawLine(
        ...
        size / 2 + ((msg_.pose.x / ocgd) +
                    real_dist *
                        cos(scan_.angle_min + i * scan_.angle_increment +
                            msg_.pose.theta) /
                        ocgd) *
                       climax,
        size / 2 + ((msg_.pose.y / ocgd) +
                    real_dist *
...
@return void
**/
float CGuiLaser::getMaxRange(void) { return msg_.maxRange; }

/**
@brief Returns the frame id of the specific laser sensor
@return std::string : The laser frame id
**/
std::string CGuiLaser::getFrameId(void) { return msg_.frame_id; }
...
@return void
**/
void CGuiLaser::setVisualizationStatus(char vs) { visualization_status_ = vs; }
}  // namespace stdr_gui
