...
@param listener [tf::TransformListener *] ROS tf listener to get the robot's
current pose
@return void
**/
void CGuiRobot::draw(QImage *m, float ocgd, tf::TransformListener *listener) {
  ...
  if (robot_initialized_ && !started_) {
    ...
    speeds_subscriber_ =
        n_.subscribe(speeds_topic.c_str(), 1, &CGuiRobot::speedsCallback, this);
  }
  started_ = true;
  resolution_ = ocgd;
  ...
  try {
    listener->waitForTransform("map_static", frame_id_.c_str(), ros::Time(0),
                               ros::Duration(0.2));
    listener->lookupTransform("map_static", frame_id_.c_str(), ros::Time(0),
                              transform);
...
void CGuiRobot::drawSelf(QImage *m) {
  ...
  if (footprint_.points.size() == 0) {
    painter.drawEllipse((current_pose_.x - radius_) / resolution_,
                        (current_pose_.y - radius_) / resolution_,
                        radius_ * 2.0 / resolution_,
                        radius_ * 2.0 / resolution_);

    painter.drawLine(
        current_pose_.x / resolution_, current_pose_.y / resolution_,
    ...
    for (unsigned int i = 0; i < footprint_.points.size() + 1; i++) {
      ...
      points[i] = QPointF(current_pose_.x / resolution_ +
    ...
    painter.drawLine(
        QPointF(current_pose_.x / resolution_, current_pose_.y / resolution_),
  ...
  if (show_circles_) {
    ...
    for (unsigned int i = 0; i < 5; i++) {
      painter.drawEllipse((current_pose_.x - (i + 1.0) / 2.0) / resolution_,
                          (current_pose_.y - (i + 1.0) / 2.0) / resolution_,
                          (i + 1.0) / resolution_, (i + 1.0) / resolution_);
    }
  }
}
...
**/
std::string CGuiRobot::getFrameId(void) { return frame_id_; }
...
@return void
**/
void CGuiRobot::setShowLabel(bool b) { show_label_ = b; }

/**
@brief Gets the show_label_ flag
@return bool : show_label_
**/
bool CGuiRobot::getShowLabel(void) { return show_label_; }

/**
@brief Gets the show_label_ flag
@return bool : show_label_
**/
void CGuiRobot::toggleShowLabel(void) { show_label_ = !show_label_; }

/**
@brief Toggles the show_circles_ flag
@return void
**/
void CGuiRobot::toggleShowCircles(void) { show_circles_ = !show_circles_; }
...
**/
geometry_msgs::Pose2D CGuiRobot::getCurrentPoseM(void) { return current_pose_; }

/**
@brief Returns the lasers number
@return int : the lasers number
**/
int CGuiRobot::getLasersNumber(void) { return lasers_.size(); }

/**
@brief Returns the sonars number
@return int : the sonars number
**/
int CGuiRobot::getSonarsNumber(void) { return sonars_.size(); }
...
@return char
**/
char CGuiRobot::getVisualizationStatus(void) { return visualization_status_; }
...
**/
void CGuiRobot::setEnvironmentalThermalSources(
    stdr_msgs::ThermalSourceVector env_) {
  for (unsigned int i = 0; i < thermal_sensors_.size(); i++) {
...
**/
void CGuiRobot::setEnvironmentalSoundSources(
    stdr_msgs::SoundSourceVector env_) {
  for (unsigned int i = 0; i < sound_sensors_.size(); i++) {
