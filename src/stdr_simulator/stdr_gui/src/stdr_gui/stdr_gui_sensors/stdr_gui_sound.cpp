...
**/
void CGuiSound::paint(QImage *m, float ocgd, tf::TransformListener *listener) {
  lock_ = true;
  QPainter painter(m);
  ...
  //~ {
  //~ for(unsigned int i = 0 ; i < env_sound_sources_.sound_sources.size() ;
  //i++) ~ { ~ if(sound_sources_.sound_sources_ids[j] ==
  //env_sound_sources_.sound_sources[i].id) ~ { ~ int x1 = pose_x / ocgd; ~ int
  //y1 = pose_y / ocgd; ~ int x2 = env_sound_sources_.sound_sources[i].pose.x /
  //ocgd; ~ int y2 = env_sound_sources_.sound_sources[i].pose.y / ocgd; ~
  ...
  painter.drawPie(pose_x / ocgd - msg_.maxRange / ocgd,

                  pose_y / ocgd - msg_.maxRange / ocgd,

                  msg_.maxRange / ocgd * 2, msg_.maxRange / ocgd * 2,
...
**/
char CGuiSound::getVisualizationStatus(void) { return visualization_status_; }
...
**/
void CGuiSound::setVisualizationStatus(char vs) { visualization_status_ = vs; }

/**
@brief Returns the frame id of the specific sensor
@return std::string : The sensor's frame id
**/
std::string CGuiSound::getFrameId(void) { return msg_.frame_id; }
