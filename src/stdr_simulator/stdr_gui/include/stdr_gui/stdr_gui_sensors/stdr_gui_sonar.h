...
class CGuiSonar {
 ...
 private:
  ...
  bool lock_;
  //!< The ROS topic to which the subscription must be made for the new values
  //!< to be taken
  std::string topic_;
  //!< The ROS tf frame
  ...
  **/
  ...
  @param m [QImage*] The image to be drawn
  @param ocgd [float] The map's resolution
  @param maxRange [float] The maximum range of all the robot sensors. Used for
  the visualizer proportions
  @return void
  **/
