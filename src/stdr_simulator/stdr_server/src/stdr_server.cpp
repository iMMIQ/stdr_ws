...
Server::Server(int argc, char **argv)
    : _spawnRobotServer(_nh, "stdr_server/spawn_robot",
                        boost::bind(&Server::spawnRobotCallback, this, _1),
                        false)

      ,
      _registerRobotServer(
          _nh, "stdr_server/register_robot",
          boost::bind(&Server::registerRobotCallback, this, _1), false)

      ,
      _deleteRobotServer(_nh, "stdr_server/delete_robot",
                         boost::bind(&Server::deleteRobotCallback, this, _1),
                         false)
  ...
  _loadMapService = _nh.advertiseService("/stdr_server/load_static_map",
                                         &Server::loadMapCallback, this);

  _loadExternalMapService =
      _nh.advertiseService("/stdr_server/load_static_map_external",
  ...
  _robotsPublisher = _nh.advertise<stdr_msgs::RobotIndexedVectorMsg>(
  ...
  _addRfidTagServiceServer = _nh.advertiseService(
      "stdr_server/add_rfid_tag", &Server::addRfidTagCallback, this);

  _deleteRfidTagServiceServer = _nh.advertiseService(
      "stdr_server/delete_rfid_tag", &Server::deleteRfidTagCallback, this);

  _rfidTagVectorPublisher =
      _nh.advertise<stdr_msgs::RfidTagVector>("stdr_server/rfid_list", 1, true);

  //!< CO2 sources

  _addCO2SourceServiceServer = _nh.advertiseService(
      "stdr_server/add_co2_source", &Server::addCO2SourceCallback, this);

  _deleteCO2SourceServiceServer = _nh.advertiseService(
      "stdr_server/delete_co2_source", &Server::deleteCO2SourceCallback, this);
  ...
  _addThermalSourceServiceServer =
      _nh.advertiseService("stdr_server/add_thermal_source",
  ...
  _addSoundSourceServiceServer = _nh.advertiseService(
      "stdr_server/add_sound_source", &Server::addSoundSourceCallback, this);
...
**/
bool Server::addRfidTagCallback(stdr_msgs::AddRfidTag::Request &req,
                                stdr_msgs::AddRfidTag::Response &res) {
  //!< Sanity check for the RFID tag
  stdr_msgs::RfidTag new_rfid = req.newTag;
  if (_rfidTagMap.find(new_rfid.tag_id) !=
      _rfidTagMap.end()) {  //!< A rfid tag exists with the same tag_id
    res.success = false;
  ...
  _rfidTagMap.insert(
      std::pair<std::string, stdr_msgs::RfidTag>(new_rfid.tag_id, new_rfid));
...
**/
bool Server::addCO2SourceCallback(stdr_msgs::AddCO2Source::Request &req,
                                  stdr_msgs::AddCO2Source::Response &res) {
  //!< Sanity check for the source
  stdr_msgs::CO2Source new_source = req.newSource;
  if (_CO2SourceMap.find(new_source.id) !=
      _CO2SourceMap.end()) {  //!< A source exists with the same id
    res.success = false;
  ...
  _CO2SourceMap.insert(
      std::pair<std::string, stdr_msgs::CO2Source>(new_source.id, new_source));
  ...
  for (CO2SourceMapIt it = _CO2SourceMap.begin(); it != _CO2SourceMap.end();
       it++) {
...
bool Server::addThermalSourceCallback(
  ...
  if (_thermalSourceMap.find(new_source.id) !=
      _thermalSourceMap.end()) {  //!< A source exists with the same id
    res.success = false;
...
**/
bool Server::addSoundSourceCallback(stdr_msgs::AddSoundSource::Request &req,
                                    stdr_msgs::AddSoundSource::Response &res) {
  //!< Sanity check for the source
  stdr_msgs::SoundSource new_source = req.newSource;
  if (_soundSourceMap.find(new_source.id) !=
      _soundSourceMap.end()) {  //!< A source exists with the same id
    res.success = false;
...
**/
bool Server::deleteRfidTagCallback(stdr_msgs::DeleteRfidTag::Request &req,
...
bool Server::deleteThermalSourceCallback(
  ...
  if (_thermalSourceMap.find(name) != _thermalSourceMap.end()) {
    // Publish deletion to RVIZ
    visualization_msgs::MarkerArray thermalMarkerArray;
    thermalMarkerArray.markers.push_back(
        toMarker(_thermalSourceMap[name], false));
    _sourceVectorPublisherRviz.publish(thermalMarkerArray);
...
void Server::spawnRobotCallback(const stdr_msgs::SpawnRobotGoalConstPtr &goal) {
  ...
  if (addNewRobot(goal->description, &result)) {
    ...
    for (RobotMap::iterator it = _robotMap.begin(); it != _robotMap.end();
         ++it) {
...
@return bool
**/
bool Server::addNewRobot(stdr_msgs::RobotMsg description,
                         stdr_msgs::SpawnRobotResult *result) {
...
@return bool
**/
bool Server::deleteRobot(std::string name,
                         stdr_msgs::DeleteRobotResult *result) {
...
visualization_msgs::Marker Server::toMarker(const stdr_msgs::CO2Source &msg,
  ...
  // Set the color specific to the source type -- be sure to set alpha to
  // something non-zero!
  marker.color.r = 0.0f;
  marker.color.g = 1.0f;
  marker.color.b = 0.0f;
...
visualization_msgs::Marker Server::createMarker(const SourceMsg &msg,
                                                bool added) {
