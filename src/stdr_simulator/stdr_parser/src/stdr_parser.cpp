...
void Parser::parse(std::string file_name) {
  ...
  try {
    ...
    std::string error = std::string("Failed to load file '") + file_name +
...
bool Parser::eliminateFilenames(Node* n) {
  ...
  for (unsigned int i = 0; i < n->elements.size(); i++) {
    if (n->elements[i]->tag ==
        "filename") {  //!< Sanity check for filename. Base and file must be the
                       //!< same
...
filename of wrong type specified\n") +
                            std::string("\nError was in line ") +
                            SSTR(n->file_row) + std::string(" of file '") +
                            n->file_origin + std::string("'");
        throw ParserException(error);
      }
