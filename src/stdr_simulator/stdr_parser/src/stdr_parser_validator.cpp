...
void Validator::validityAllowedCheck(std::string file_name, Node* n) {
  ...
  if (Specs::specs.find(tag) == Specs::specs.end() &&
    ...
    std::string error = std::string("STDR parser : ") + n->tag +
  ...
  for (unsigned int i = 0; i < n->elements.size(); i++) {
    ...
    if (tag != "STDR_Parser_Root_Node" && child_value == "") {
      if (Specs::specs[tag].allowed.find(child_tag) ==
        ...
        int decreaser = (extractFilename(n->elements[i]->file_origin) ==
                             ...
                             ? 1
                             : 0);
        std::string error = std::string("STDR parser : ") + child_tag +
                            std::string(" is not allowed in a ") + tag +
    ...
    try {
      ...
      int decreaser =
          (extractFilename(n->file_origin) == extractFilename(file_name) ? 1
                                                                         : 0);
      std::string trail(ex.what());
      trail += std::string("\n  [") + n->tag + std::string("] Line ") +
               SSTR(n->file_row - decreaser) + std::string(" of file '") +
...
void Validator::validityRequiredCheck(std::string file_name, Node* n) {
  ...
  for (std::set<std::string>::iterator it = Specs::specs[tag].required.begin();
    ...
    if (num.size() == 0) {
      std::string error = std::string("STDR parser : ") + tag +
...
void Validator::parseMergableSpecifications(void) {
  ...
  if (!loadOkay) {
    std::string error = std::string("STDR parser : Failed to load file ") +
                        path + std::string("'") +
                        std::string("\nError was \n\t") +
                        std::string(doc.ErrorDesc());
    throw ParserException(error);
  }
  Specs::non_mergable_tags =
      explodeString(doc.FirstChild()->FirstChild()->Value(), ',');
}
    ...
    case 1:  //!< Type = element
    {
      if (node_text == "specifications") {
        break;
                 ...
                 node_text != "default") {  //!< Base specifications tag
        if (Specs::specs.find(node_text) ==
            Specs::specs.end()) {  //!< New base specifications tag
          Specs::specs.insert(
              std::pair<std::string, ElSpecs>(node_text, ElSpecs()));
        } else {  //!< Multiple base tags - error
          std::string error =
              std::string("STDR parser : Multiple instance of '") + node_text +
              std::string("' in specifications.");
          throw ParserException(error);
        }
      } else {  //!< required or allowed tags
        break;
      }
      break;
    }
