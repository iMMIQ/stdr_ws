...
class odomObserve {
 public:
  odomObserve() {
    ...
    marker_pub =
        n.advertise<visualization_msgs::MarkerArray>("MarkerArray", 20);
  ...
  bool checkIfGet(int i) { return flags[i]; }
  void rectChange(float x, float y, int i) {
    if (x < rect[i][0] + 0.25 && x > rect[i][0] - 0.25 &&
        y < rect[i][1] + 0.25 && y > rect[i][1] - 0.25) {
